...
- **Fit vs fill vs stretch** (`FitMode` in `rasterise.ts`) answers a third, unrelated question: how does the _source SVG's own content_ map into the export raster during rasterisation. `'fit'` (default, unchanged from v1) sets `preserveAspectRatio="xMidYMid meet"` — the content letterboxes/pillarboxes, undistorted and centred, leaving background-filled space on one axis when the SVG's own aspect ratio differs from the export raster's. `'fill'` sets `preserveAspectRatio="xMidYMid slice"` — the content is scaled to _cover_ the raster completely (no empty space on either axis), cropping whatever overflows the non-limiting axis, undistorted — added after real usage found `'fit'` alone unsatisfying for a source whose aspect differs a lot from the DVD target (e.g. an 800×600 scene into 720×480 leaves visible pillarbox bars; `'fill'` crops instead of bordering). `'stretch'` sets `preserveAspectRatio="none"` — the content fills the raster exactly, distorting if the aspect ratios differ. All three are a direct one-line mapping onto SVG's own `preserveAspectRatio` spec; the browser does the actual scaling (and, for `'fill'`, cropping).
...
**`ResultPanel`'s in-app preview only attempts playback for `h264-mp4`.** `RenderResult` now carries the `codec` actually used, precisely so the frontend can know this authoritatively rather than probing it the hard way. MPEG-2 in an MPEG-PS container (`mpeg2-dvd`) isn't a web-playable format in any browser, at any layer — this is not the loading-mechanism gap §4.8's blob fallback exists for, it's a decoder-support gap no amount of streaming-vs-blob trickery fixes. FFV1 and QuickTime RLE are lossless intermediates for further processing, not meant for casual preview either. For any codec outside that allow-list, the `<video>` element is never rendered at all (avoiding a doomed load attempt and, worse, a wasted full-file blob fetch) — instead a clear message points at Reveal/Save and the ffprobe report, which still proves the file is valid even though it can't play inline.
